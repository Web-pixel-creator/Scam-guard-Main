...
- [ ] 1. Настройка тестов, миграции БД и серверных секретов
  - [ ] 1.1 Добавить и настроить Vitest + fast-check
  ...
  - [ ] 1.2 Создать таблицу `telegram_sessions` (Supabase, service-role)
  ...
  - [ ] 1.3 Добавить серверные переменные окружения бота
...
- [ ] 2. Вынести общее ядро проверки `check-core.ts`
  - [ ] 2.1 Реализовать `runCheck` и `ocrExtractCore`
  ...
  - [ ]* 2.2 Property-тесты ядра: детерминизм и приватность
  ...
  - [ ]* 2.3 Unit-тест: веб-контракт `checkInput` не изменился
    - Проверить, что ключ rate-limit остался `check:<ip>` и формат ответа идентичен дорефакторному
    - _Requirements: 4.2_

- [ ] 3. Расширить `rules.ts` новыми reason codes (локальные сценарии)
  - [ ] 3.1 Добавить 4 reason codes без поломки порогов
  ...
  - [ ]* 3.3 Unit-тесты regex новых кодов (RU/UZ, позитив/негатив)
...
- [ ] 4. Реализовать session store `session.server.ts`
  - [ ] 4.1 Реализовать загрузку/сохранение сессии
  ...
  - [ ]* 4.2 Unit/интеграционные тесты сессии (мок `supabaseAdmin`)
...
- [ ] 5. Реализовать Telegram API helper `api.server.ts`
  - [ ] 5.1 Реализовать обёртку Bot API и `escapeMarkdownV2`
  ...
  - [ ]* 5.2 Property-тест MarkdownV2-безопасности
  ...
  - [ ]* 5.3 Unit-тесты `getFile`/`downloadFileAsDataUrl` (мок `fetch`)
    - Файл > 6 МБ не скачивается и отклоняется; валидный файл → data URL в памяти; форма запросов к Bot API
    - _Requirements: 5.3, 5.5_

- [ ] 6. Реализовать bot-i18n и форматтер ответа
  - [ ] 6.1 Добавить трилингвальные строки бота
...
- [ ] 7. Чекпоинт — ядро, helpers и форматтер
  - Прогнать все тесты, при вопросах обратиться к пользователю.

- [ ] 8. Реализовать роутер обновлений и обработчики
  - [ ] 8.1 Реализовать `router.ts` (диспетчер + `parseCommand`)
  ...
  - [ ] 8.2 Реализовать командные обработчики (`commands`)
  ...
  - [ ] 8.3 Реализовать обработчики проверки (`check`)
  ...
  - [ ] 8.4 Реализовать сценарий жалобы (`report`)
  ...
  - [ ] 8.5 Реализовать callback- и fallback-обработчики (`misc`)
  ...
  - [ ]* 8.6 Property-тест ключа rate-limit
  ...
  - [ ]* 8.7 Property-тест эквивалентности контакт-карты
  ...
  - [ ]* 8.9 Интеграционный тест сценария жалобы
...
- [ ] 9. Реализовать webhook route и интеграцию
  - [ ] 9.1 Реализовать `webhook.ts` (server route)
  ...
  - [ ]* 9.2 Property-тесты контракта webhook
  ...
  - [ ]* 9.3 Интеграционные тесты webhook end-to-end (моки Telegram API + `supabaseAdmin`)
  ...
  - [ ]* 9.4 Интеграционный тест деградации AI
...
- [ ] 10. Развёртывание webhook
  - [ ] 10.1 Утилита регистрации webhook + инструкции
