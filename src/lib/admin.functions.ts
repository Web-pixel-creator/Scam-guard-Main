...
export const listEntities = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context.userId);
    let q = supabaseAdmin.from("entities").select("*").order("last_seen_at", { ascending: false }).limit(200);
...
export const moderateReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rep, error } = await supabaseAdmin
      .from("reports").select("entity_hash, entity_type, redacted_value")
      .eq("id", data.reportId).maybeSingle();
    ...
    const { data: ent } = await supabaseAdmin.from("entities")
      .select("id").eq("entity_hash", rep.entity_hash).maybeSingle();
    if (ent) {
...
export const adminStats = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const [reportsNew, reportsConfirmed, entitiesConfirmed, checksTotal] = await Promise.all([
      ...
      supabaseAdmin.from("reports").select("id", { count: "exact", head: true }).eq("status", "confirmed"),
      supabaseAdmin.from("entities").select("id", { count: "exact", head: true }).eq("moderation_status", "confirmed"),
