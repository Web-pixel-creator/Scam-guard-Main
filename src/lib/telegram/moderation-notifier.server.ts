...
export function formatModerationNoticeForTelegram(notice: ModerationNotice): string {
  ...
  if (notice.kind === "appeal") {
    return [
      ...
      "Короткое уведомление для команды проверки.",
      "",
      "📌 Что проверить",
      `• Объект: ${targetLabel(notice)}`,
      `• Язык: ${notice.language}`,
      "",
      "Решение, статус и история цели доступны только в админке после входа.",
      "Не отправляйте в этот чат коды, карты, пароли, скриншоты или полные контакты.",
    ].join("\n");
  }
  ...
  const lines = [
    ...
    "",
    notice.duplicateOfExisting
      ? "ℹ️ На эту цель уже жаловались сегодня. Мы не создаём дубль, но учитываем повторный сигнал: модератору стоит присмотреться."
      : null,
    notice.duplicateOfExisting ? "" : null,
    "🔐 Почему номер/username скрыт",
    "В этот Telegram-чат уходит только маска. Полный номер/username, решение и история цели доступны в админке после входа.",
    "",
    "Не отправляйте в этот чат коды, карты, пароли, скриншоты или полные контакты.",
