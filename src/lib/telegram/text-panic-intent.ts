...
const ACCIDENTAL_OUTGOING_TRANSFER_ACTION_RE =
  /(?:^|[^\p{L}])(?:я|мы)\s+(?:уже\s+|только\s+что\s+)?(?:перев[её]л[аи]?|перв[её]л[аи]?|переводил[аи]?|заплатил[аи]?|оплатил[аи]?|оплачивал[аи]?|пополнил[аи]?|пополнял[аи]?)(?!\p{L})|(?:^|[^\p{L}])(?:перев[её]л[аи]?|перв[её]л[аи]?|переводил[аи]?|заплатил[аи]?|оплатил[аи]?|оплачивал[аи]?|пополнил[аи]?|пополнял[аи]?)(?!\p{L})|(?:плат[её]ж|перевод)\s+(?:уже\s+)?(?:исполнил\p{L}*|прош[её]л|зачислен\p{L}*)|(?:^|[^\p{L}])(?:i|we)\s+(?:already\s+|just\s+)?(?:transferred|transerred|wired|paid|topped\s+up|recharged)(?!\p{L})|(?:payment|transfer)\s+(?:already\s+)?(?:settled|completed|went\s+through)|(?:^|[^\p{L}])(?:men|biz)(?!\p{L}).{0,100}(?:o['’]?tkazdim|otkazdim|to['’]?ladim|to['’]?lov\s+qildim)(?!\p{L})|(?:^|[^\p{L}])(?:o['’]?tkazdim|otkazdim|to['’]?ladim|to['’]?lov\s+qildim)(?!\p{L})|(?:to['’]?lov|o['’]?tkazma)\s+(?:o['’]?tib\s+bo['’]?ldi|o['’]?tdi|bajarildi)/iu;
