...
describe("classifyVictimIntent", () => {
  it.each([
    ...
    ["мне звонят с другой страны", "unknown_call"],
    ["просто звонок с другой страны брать трубку?", "unknown_call"],
    ["мне звонят из Нигерии", "unknown_call"],
    ["звонят с номера +98 из Ирана", "unknown_call"],
    ...
    ["звонят из водоканала и просят паспорт для умного счетчика", "authority_impersonation"],
    ["пенсионный фонд обещает повысить пенсию и просит данные карты", "authority_impersonation"],
    ["инспектор МИБ требует наличные за списание долга", "authority_impersonation"],
    ...
    ["покупают голос Open Budget и просят SMS код", "code_request"],
    ["врач DMED просит SMS код для записи", "code_request"],
    ["ребенку предлагают бесплатные бонусы в игре и просят код", "code_request"],
    ["menga nimadir yuborishdi", "telegram_message"],
    ["мне пишут от имени Telegram с галочкой", "link_request"],
    ["официальный Telegram просит пройти проверку иначе аккаунт удалят", "link_request"],
    ["мне прислали APK я ухожу из этого мира", "file_received"],
    ["оповещение Apple iOS повреждена на 72 процента просит установить защиту", "apk_request"],
    ["незнакомец просит телефон на минуту позвонить", "general_scam_concern"],
    ["деньги пришли по ошибке просят вернуть на другой счет", "transfer_request"],
    ["звонят и молчат чтобы записать голос", "unknown_call"],
