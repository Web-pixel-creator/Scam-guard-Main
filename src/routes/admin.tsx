...
function AdminPage() {
  ...
  const policyFn = useServerFn(getAdminAuthPolicy);
  ...
  const authPolicy = useQuery({
    queryKey: ["admin-auth-policy", user?.id],
    enabled: !!user && isAdmin,
    queryFn: () => policyFn({ data: undefined as never }),
    retry: 1,
  });
  const mfaRedirectRequired =
    authPolicy.data !== undefined && adminSessionDestination(authPolicy.data) === "/admin-mfa";
  const adminDataEnabled =
    !!user && isAdmin && authPolicy.isSuccess && authPolicy.data.isAdmin && !mfaRedirectRequired;

  useEffect(() => {
    if (mfaRedirectRequired) {
      void nav({ to: "/admin-mfa", replace: true });
    }
  }, [mfaRedirectRequired, nav]);
  ...
  if (loading || (!!user && isAdmin && authPolicy.isLoading) || mfaRedirectRequired) {
    return (
      <div className="apex-page">
        <div className="apex-card apex-frame apex-stripes text-center">
          <span className="apex-mono inline-flex items-center gap-2 text-[#52525B]">
            ...
            {mfaRedirectRequired ? "ПЕРЕХОД К MFA…" : "ПРОВЕРКА ДОСТУПА…"}
          </span>
        </div>
      </div>
    );
  ...
  if (!user) return null;
  if (authPolicy.isError) {
    return (
      <div className="apex-page" style={{ maxWidth: 560 }}>
        <div className="apex-card apex-frame apex-stripes text-center">
          <p className="label-md mb-3">ADMIN / AUTH</p>
          <h1 className="apex-h1" style={{ fontSize: "clamp(22px, 5vw, 32px)" }}>
            Не удалось проверить <span className="font-serif-italic text-[#B91C1C]">доступ</span>
          </h1>
          <p className="apex-lead mt-3 sm:mt-4 mx-auto">
            Защищённые данные не загружались. Повторите проверку или войдите заново.
          </p>
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <button
              type="button"
              onClick={() => void authPolicy.refetch()}
              className="apex-btn-outline inline-flex items-center gap-2"
            >
              <RefreshCcw className="h-3.5 w-3.5" aria-hidden="true" /> Повторить
            </button>
            <button
              type="button"
  ...
  if (!isAdmin || authPolicy.data?.isAdmin === false) {
    return (
      ...
      <main className="admin-shell">
        <header className="admin-topbar">
          ...
          <div className="admin-top-actions">
            <span className="admin-live">
              <i />{" "}
              {authPolicy.data?.currentAal === "aal2" ? "MFA подтверждена" : "Сессия по паролю"}
            </span>
        ...
        <section className="admin-hero">
          ...
          <div className="admin-shift-card">
            ...
            <strong>
              {authPolicy.data?.currentAal === "aal2" ? "MFA подтверждена" : "Базовая сессия"}
            </strong>
            ...
            <div className="admin-shift-actions">
              ...
              <button type="button" onClick={() => void nav({ to: "/admin-mfa" })}>
                <LockKeyhole aria-hidden="true" /> MFA / безопасность
              </button>
