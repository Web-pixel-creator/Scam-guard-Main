...
- Public forwarded Telegram channel/group source context is presentation-only. The router may pass a sanitized source title/public username into the reply so users understand where a forwarded post came from, but source metadata is not appended to `runCheck` input, does not affect score/level/reasons and is not persisted in `checks`.
