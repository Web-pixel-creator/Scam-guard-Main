...
- **AI:** Provider-neutral OpenAI-compatible Chat Completions plus optional audio transcription. `OPENAI_API_KEY` enables explanations, screenshot OCR/image understanding and Telegram voice STT; `OPENAI_MODEL`, `OPENAI_BASE_URL` and `OPENAI_TRANSCRIBE_MODEL` are optional.
...
4. Telegram voice-note path: `handleVoice` downloads short voice files only in memory, calls `transcribeVoiceCore`, redacts/clips the transcript and then runs the same `runCheck` pipeline. Raw audio is never stored.
...
6. `runCheck` performs shared rate-limit, input detection, normalization, display masking, `redactText`, rule evaluation, entity lookup, scoring, optional AI explanation and a redacted `checks` insert. For phone inputs it also builds an honest `PhoneIntelligencePassport` with country/calling-code, Uzbekistan prefix/operator hints and official-directory status; this is explanatory metadata and does not claim an owner. If a phone `entities` row is confirmed, it also returns `PhoneReputationSummary` with Ishonch Guard moderated report count/confidence only.
...
- Short Telegram voice notes are downloaded in memory, capped before transcription, transcribed through the configured AI provider and discarded. Only the redacted transcript is sent into `runCheck`; when STT is unavailable the bot gives an actionable fallback instead of pretending to understand the audio.
