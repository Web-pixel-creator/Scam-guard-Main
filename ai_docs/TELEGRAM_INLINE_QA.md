...
## Exploratory Desktop evidence — 2026-07-15

The owner supplied 41 Telegram Desktop screenshots covering preview, insertion
and direct-bot follow-up behavior. Exact local copies are stored under
`private/telegram-inline-qa/2026-07-15/desktop/user-batch-01/`; the grouped
manifest and defect-to-fix mapping are in
`TELEGRAM_INLINE_CLIENT_AUDIT_2026-07-15.md`.

This batch is **pre-fix exploratory evidence**, not a passing client matrix.
It exposed job-fee copy falling through to generic money-transfer advice,
...
Current counted real-client progress remains 1/17 Telegram Desktop cases and
1/51 total client rows passed from the 2026-07-14 post-fix pair. The 2026-07-15
batch does not add a pass. Android and iOS remain at 0/17 each.
...
- changing the query refreshes the corresponding result instead of leaving a
  stale first result, and ordinary transient delivery does not silently vanish;
...
| INLINE-PHONE-SHORT      | `@scamguard_bot 12345678`                                                                   | Preview says `Код или неполный номер` and tells the user not to paste a real code.          | Inserted card asks for a complete phone plus a worded description of the request.                 | No country/owner inference, digit echo or safety verdict.                          |
...
| INLINE-LINK-MULTILINE   | `@scamguard_bot Мне пишет мошенник` + newline + `Спрашивает ссылку которую он мне отправил` | Preview prioritizes the added link context and says not to open it yet.                     | Inserted card asks for the actual URL or complete request without inventing a check.              | No unchanged first-line-only answer.                                               |
...
| INLINE-RATE-LIMIT       | Repeated inline queries from the same account until limited                                 | Preview says `Слишком много проверок` and includes current retry seconds.                   | No message is sent to the chat; the retry card is not reused from a stale cache.                  | No raw error, stack trace, vague wait-only copy or reset caused only by cached UI. |
| INLINE-TRANSIENT        | Change one ordinary query to a second different query                                       | The second preview replaces the first within normal client timing.                          | The selected second result inserts the second query's card.                                       | No stale first result and no silent empty list after a recoverable delivery error. |
...
- query order and approximate timestamp for stale-result/rate-limit cases;
...
- Job-fee, passport, multiline-link and ambiguous-numeric cases use their own
  specific guidance instead of a generic transfer/phone fallback.
...
- A second changed query produces its own result; a retryable Bot API failure is
  not silently acknowledged as delivered. The forced timeout/429/5xx branch is
  still automated-only and must not be manufactured in production.
