...
| `.kiro/specs/*/`                      | Feature specs for Telegram UX, brand impersonation, meta intent, image/link intelligence, inline check, menu/result polish, emergency copilot, response compression, phone directory, webhook dedup, shared rate limits, research-feed and website trust/trends/impact work. |
...
`CheckInput`, `RiskResultCard`, `QuickReportForm`, `StatsStrip`, `HomeImpactCounters`, `HomeTrustSurface`, `HomeSchemeTrends`, `OfficialContactsDirectory`, `SchemeTrendsPanel`, `Layout`, `LanguageSwitcher`, `A11yPanel`, `FancyButton`, `UnicornBackground`; `components/ui/*` are shadcn primitives.
