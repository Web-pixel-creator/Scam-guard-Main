...
- `ApprovedRussianHomepage()` composes the exact approved Russian landing page
  ...
  counters. Local and deployed hosts keep real loading/all-zero states and do
  not substitute remembered production values.
- `CheckInput({ variant: "signal" })` and
  `QuickReportForm({ variant: "signal" })` reuse the production server
  functions inside the exact checker and community-report presentation. The
  signal checker auto-detects phone/Telegram/URL/message input, keeps screenshot
  OCR separate and marks the prefilled verdict as a demo.
...
- `runCheck(params)` is the transport-independent check pipeline. It uses the shared production limiter with a local fallback. `persist:false` is reserved for non-final previews such as Telegram inline typing and skips the `checks` insert while preserving deterministic scoring. Verified metadata, phone passport and Safe require an exact standalone contact; embedded official tokens are not trusted. Composite text/image payloads retain embedded destinations for deterministic URL/brand scoring. Local deterministic text rules inspect the complete in-memory prose after embedded URLs are replaced by `[link]`; only the redacted form may cross AI, persistence and presentation boundaries.
...
- `detectTelegramSensitiveSecret(original)` and
  ...
  password, private-key and recovery material after sink sanitization and
  render containment guidance without echoing the value. Detection includes
  bounded mixed-script/spaced/typo labels, formatted/value-first alphanumeric
  codes and canonical recovery lists; its value-first scan is linear rather
  than an unanchored superlinear regex.
- `hasPastedSensitiveSecretValue(original)` distinguishes an actual supplied
  value from natural completed-action wording so secret preflight remains
  authoritative without suppressing urgent aftercare.

**`src/lib/telegram/report-flow.ts`**

- `withReportCallbackBinding(...)` writes an exact message/action/scenario
  binding with canonical UTC time. `matchesReportCallbackBinding(...)`
  accepts it for 20 minutes and fails closed for malformed/future/expired,
  cross-message, cross-action or cross-scenario callbacks.
...
**`src/lib/telegram/inline-adapted-dialogue-corpus.ts`**

- `INLINE_ADAPTED_DIALOGUE_CORPUS` converts every turn from the 1,000-dialogue
  perimeter plus contextual follow-ups, mixed clauses and synthetic credential
  boundaries into 3,805 bounded stateless Inline cases / 2,140 unique queries.
- `INLINE_ADAPTED_DIALOGUE_STATS` records exact source and RU/UZ/EN counts.
  Construction is pure; the companion handler test denies external fetch and
  database writes and never sends the corpus to Telegram or an AI provider.
...
- `src/lib/telegram/polling-cycle.ts`: dependency-injected batch core. It validates
  every `update_id` in strict increasing order before any side effect, keeps
  stateful/unsupported records in causal order, and processes strict-Inline-only
  work in just-in-time lifecycle chunks of at most four. A slow stateful update
  may overlap only with following Inline updates for known different users;
...
- `src/lib/telegram/update-dispatch.server.ts`: preserves per-user serialization for webhook and stateful work. An AsyncLocalStorage option set by the polling supervisor bypasses it only for strict Inline-only updates; message/callback hybrids ignore the option.
...
- Secret-bearing Voice transcripts in `handlers/check.ts` use a fully static
  ...
  both the 24-hour replay cache and raw shared in-flight results.
...
- Secret queries in `handlers/inline.ts` return a static private result with no
  user-derived preview or inserted context; the normal stateless check path is
  not entered.
