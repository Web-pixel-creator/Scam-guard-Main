...
- `src/lib/telegram/image-fallback.ts`: builds `imgtriage:*` callback data, unreadable-image triage keyboards and compact hook/risk/safe-step copy; it is presentation-only and does not run scoring or persistence.
