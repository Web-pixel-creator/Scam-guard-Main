...
| Voice-out | `voiceout:panic:<id>`, `voiceout:guardian` | Provider/quota/chat-noise side effect; safe only with approval | `voice-out.server.test.ts`, `emergency-followup.test.ts`, `webhook.integration.test.ts`, `tts:validate-assets`, `prod:telegram-voice-out-smoke` | Automated release QA passed again on 2026-07-04 for 45 static OGG assets and the focused Telegram suite. With approval, production smoke passed for RU/UZ/EN `panic-6` sendAudio plus webhook callback; full human listen-through remains optional |
...
| Result: "What to do urgently" | Pass | Opened the emergency "Что произошло?" menu with concrete next-situation buttons and no external side effect. |
| Result: "New check" | Pass | Returned the prompt for a new object: number, username, link, message text, or screenshot. |
